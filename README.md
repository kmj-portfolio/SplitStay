...
- 기존 호텔 예약 서비스는 개인 중심으로 설계되어 있어 여럿이 함께 여행을 계획할 때 비용 정산과 의사결정 과정이 번거롭다는 문제를 해결하고자 개발했습니다. 
...
- Bounding Box + 복합 인덱스 + ST_Distance_Sphere()로 최적화한 기반 거리 검색
- QueryDSL을 사용해 체크인, 체크아웃, 가격, 성급, 인원으로 필터링
...
- 웹훅 기반 상태 변경으로 데이터 정합성 확보
- Server-Sent Events (SSE) 도입으로 결제 완료 시 실시간으로 클라이언트에 전달 
- 멱등성 처리 + 비관적락으로 중복 결제 및 동시성 문제 방지
